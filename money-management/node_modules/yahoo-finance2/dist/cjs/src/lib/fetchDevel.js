...
async function fetchDevel(url, fetchOptions) {
    if (process.env.FETCH_DEVEL === "nocache")
        return await node_fetch_1.default(url, fetchOptions);
    ...
    */
    ...
    try {
        contentJson = await fs_1.default.promises.readFile(destUrl, { encoding: "utf8" });
        contentObj = JSON.parse(contentJson);
    }
    catch (error) {
        if (error.code === "ENOENT") {
            const res = await node_fetch_1.default(url, fetchOptions);
            contentObj = {
                request: {
                    url: url,
                },
            ...
            await fs_1.default.promises.writeFile(destUrl, contentJson, { encoding: "utf8" });
        }
        else {
            throw error;
        }
